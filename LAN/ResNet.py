...
class ResNet(nn.Module):
    ...
    def _forward(self, x, with_latent=True, fake_relu=False, no_relu=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def _resnet(block, layers, num_classes, **kwargs):
    ...
    #         pretrain_dict = torch.load(cfg.MODEL.PRETRAIN_PATH)
    #         model_dict = model.state_dict()
    #         pretrain_dict = {k: v for k, v in pretrain_dict.items() if
    #                          k in model_dict and model_dict[k].size() == v.size()}
    #         model_dict.update(pretrain_dict)
    #         model.load_state_dict(model_dict)
    #         print('Initialized {} with pretrained weights from {}'.format(model_name, cfg.MODEL.PRETRAIN_PATH))
    return model
